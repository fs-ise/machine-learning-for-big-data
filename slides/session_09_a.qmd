---
title: "Machine learning for non-tabular data: image, video, and audio"
title-slide-image: "../images/cover_slide/ml-image-audio-video.png"
...
The tidymodels `mlp()` specification provides a basic feedforward network for
classification or regression. Its engine determines which architectures are available.
...
library(tidymodels)
set.seed(42)
split <- initial_split(data,prop=.75,strata=outcome)
mlp_spec <- mlp(hidden_units=32,penalty=.0001,epochs=200) |>
  set_engine("nnet",MaxNWts=10000) |> set_mode("classification")
model <- workflow() |> add_formula(outcome ~ .) |> add_model(mlp_spec) |>
  fit(training(split))
```

## torch for R (I)

For advanced neural networks and production environments, the R `torch` package provides tensors, modules, and automatic differentiation.

::: {.tall-code-box}

```r
library(torch)
X <- torch_randn(100,10); y <- X$sum(dim=2,keepdim=TRUE)
model <- mlp(10,c(32,16),1)
optimizer <- optim_adam(model$parameters,lr=1e-3)
for(epoch in 1:100){
  optimizer$zero_grad(); predictions <- model(X)
  loss <- nnf_mse_loss(predictions,y); loss$backward(); optimizer$step()
...
library(torch)
mlp <- nn_module(
  initialize=function(input_dim,hidden_layers,output_dim){
    dimensions <- c(input_dim,hidden_layers,output_dim)
    layers <- list()
    for(i in seq_len(length(dimensions)-1)){
      layers <- c(layers,list(nn_linear(dimensions[i],dimensions[i+1])))
      if(i<length(dimensions)-1) layers <- c(layers,list(nn_relu()))
    }
    self$model <- do.call(nn_sequential,layers)
  },
  forward=function(x) self$model(x)
)
model <- mlp(input_dim=10,hidden_layers=c(32,16),output_dim=1)
```
:::

::: aside
See the R torch documentation for `nn_module()` and its inherited methods.
:::
