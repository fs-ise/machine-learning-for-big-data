...
The workflow separates model development from reliable evaluation on unseen data.
...
library(tidyverse)
set.seed(42)
d <- tibble(x=seq(0,10,length.out=200),y=5-1.2*x+rnorm(200))
p <- ggplot(d,aes(x,y))+geom_point(alpha=.15)
for(i in 1:80){
  cutoff <- runif(2,0,10); sample <- filter(d,x>min(cutoff),x<max(cutoff))
  if(nrow(sample)>5 && nrow(sample)<60) p <- p+geom_smooth(data=sample,method="lm",se=FALSE,colour="green",alpha=.35)
}
p+geom_smooth(method="lm",se=FALSE,colour="red",linewidth=1.2)+
  labs(title="Illustration of selection bias (linear regression)")+theme_minimal()
```
...
districts <- opq(
  ...
  osmdata_sf()
districts <- districts$osm_multipolygons |>
  filter(!is.na(name)) |>
  distinct(name, .keep_all=TRUE) |>
  mutate(price_region=case_when(
    name %in% c("Westend-Süd","Westend-Nord","Innenstadt","Sachsenhausen-Nord") ~ "Expensive",
    name %in% c("Bornheim","Bockenheim","Nordend-West","Nordend-Ost") ~ "Medium",
    name %in% c("Höchst","Fechenheim","Nied","Griesheim") ~ "Affordable",
    TRUE ~ "Other"))
pal <- colorFactor(c("#4575b4","#d73027","#fee08b","#cccccc"),districts$price_region)
leaflet(districts)|>addProviderTiles("OpenStreetMap")|>
  addPolygons(fillColor=~pal(price_region),fillOpacity=.3,weight=1,popup=~paste0("<b>",name,"</b><br>Price region: ",price_region))|>
  addLegend("topright",pal=pal,values=~price_region,title="House Price Regions")
```
