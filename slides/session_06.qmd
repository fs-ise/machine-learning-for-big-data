...
title-slide-image: "../images/cover_slide/workflow.png"
section_label: "Machine Learning Foundations: Workflow"
session: "Session 6"
date: "2026-09-18"
format: revealjs
---
...
- **Connect** conceptual machine learning procedures to R implementations, including preprocessing, model training, and evaluation using tidymodels. (*see exercise*)
:::
...
Our focus will be on **supervised machine learning**:
...
## Powerful machine learning models increase the risk of overfitting
...
<!--
To avoid overfitting, the goal is not to fit the training data as closely as possible, but to maximize predictive performance on unseen data (generalization).
...
::: {.highlight_must_learn}
![](../images/best_fit_vs_generalization.png){fig-align=center width="70%"}
:::
...
- **Bias**: Systematic prediction error resulting from model assumptions or limitations (e.g., fitting a linear model to a nonlinear relationship).
- **Variance**: Sensitivity of the fitted model to changes in the training data.
  High variance means different training samples can produce substantially different predictions.
- **Noise**: Unpredictable variation in the outcome that cannot be explained by the available inputs.

More flexible models can reduce bias but often increase variance. The objective is to balance both to minimize prediction error on unseen data.
...
The workflow separates model training from model testing on unseen data.
Preprocessing, model selection, and validation must preserve that separation.
...
## Problems with using test data for model development
...
<span style="color: red;">1. Optimize models using test data.</span>

<span style="color: blue;">2. Select models based on test performance.</span>

This compromises independent evaluation and can result in:

- **Test-set leakage:** Test information influences model development.
- **Selection bias:** Selecting the model with the best test performance can produce overly optimistic estimates.

**Rule: NEVER use test data for model development or selection. Reserve it for independent final evaluation!**
...
::: {.highlight_must_learn}
![](../images/addressing_test_set_contamination.png){width=60% fig-align=center}
:::
...
```{python}
#| label: selection-bias-import-numpy-as-np
...
::: {.highlight_must_learn}
![](../images/cross_validation_grid_search.png){width=65% fig-align=center}
:::


## Cross-validation and grid search in R

```r
library(tidymodels)
# 1. Specify the model
model_spec <- logistic_reg(
  penalty = tune(),  # Strength of hyperparameter regularization (λ)
  mixture = 1        # Type: 1 = Lasso, 0 = Ridge
) |>
  set_engine("glmnet") |>
...
model_workflow <- workflow() |>
  add_recipe(model_recipe) |>  # Pre-processing specification
  add_model(model_spec)

# 4. Evaluate 20 hyperparameter values across the folds
...
- If categorical type is needed or more informative, discretize numerical features (e.g., income → low / high classes)
...
![](../images/feature_engineering_dummy.png){fig-align=center width=40%}
...
<!--
The map is produced with `osmdata`, `sf`, and `leaflet`.
...
```{python}
#| label: example-of-feature-engineering-ii-import-geopandas-as-gpd
#| fig-align: center
...
![](../images/feature_engineering_geo_gisco.png){width=85% fig-align=center}
...
# Problem sets {data-stack-name="Problem sets"}

## Spot the problem (I)

**Discussion**: Examine the code and ML workflow. Identify potential evaluation problems and explain how you would address them.
...
```r
# Identify the strongest predictor (highest absolute correlation with the target)
features <- c("income", "debt", "age")
scores <- abs(cor(loans[features], loans$default))
best_feature <- features[which.max(scores)]

model <- glm(
  reformulate(best_feature, response = "default"),
  data = train,
  family = binomial()
)
```

## Spot the problem (III)
...
```r
model_a <- glm(default ~ income,
               data = train, family = binomial())

model_b <- glm(default ~ income + debt + age,
               data = train, family = binomial())

accuracy_a <- mean(
  (predict(model_a, test, type = "response") >= 0.5) ==
    test$default
)

accuracy_b <- mean(
  (predict(model_b, test, type = "response") >= 0.5) ==
    test$default
)

# Report the best result
reported_accuracy <- max(accuracy_a, accuracy_b)
```
