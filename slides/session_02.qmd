---
title: "Analytics and Data Preparation"
title-slide-image: "../images/cover_slide/preparation.png"
...
## Why did machine learning become so powerful?

Modern machine learning and AI for big data became powerful through three mutually reinforcing developments:

1. **More computing power** make it possible to process large datasets and train more demanding models.
2. **New algorithms** enable more powerful analytical models and new application areas.
3. **New architectures** optimize data processing and computation.
4. **New analytics processes** make analytical work more mature, pervasive, and standardized.
...
See how $400 million machines are constructed to build modern chips: <https://www.youtube.com/watch?v=MiUHjLxm3V0>.
:::
...
## Algorithmic and analytical models across disciplines
...
Breakthroughs in artificial intelligence (AI)^[AI systems are based on algorithms; in the following, we use the broader term AI when referring to such algorithm-based systems.] show how new algorithms can rapidly surpass human performance.
...
- Uses **deep neural networks and reinforcement learning**
...
```{python}
#| label: jagged-frontier-of-ai-import-numpy-as-np
#| echo: false

from sys import path

path.insert(0, "../images")
from _ai_jagged_frontier import plot_ai_jagged_frontier

plot_ai_jagged_frontier()
```
:::
...
Data architectures have evolved as organizations seek to support **more diverse data and increasingly advanced analytics**.
...
- Raw data retained at scale
- Supports exploration and ML
- **Focus:** flexibility
:::
...
- Data remains across domains
- Shared access and standards
- Data products / data mesh
- **Focus:** distributed ownership
:::

::: {.column width="25%"}
**AI-ready architecture**

- Data + models + AI services
- Real-time and unstructured data
- ML pipelines, vectors, RAG
- **Focus:** operationalizing AI
:::
...
➡ **Architectural evolution reflects changing business trade-offs:**  
**consistency → flexibility → decentralization → AI integration**
...
|-------------|---|
| How many customers cancelled their subscriptions last month? | |
| Which customers are likely to cancel their subscriptions next month? | |
| Offer a 15% discount to customers with a high predicted churn risk. | |
| What were our highest-selling products last quarter? | |
| How much demand should we expect for each product next month? | |
| Reorder 5,000 units to minimize the risk of stock-outs. | |
| Which regions had the highest employee turnover last year? | |
| How many employees are likely to leave during the next 12 months? | |
| Increase recruiting capacity by 20% to maintain required staffing levels. | |
...
::: aside
Alternative models include the [Data Science in R workflow](https://r4ds.hadley.nz/whole-game.html),
the [KDD Process](https://de.wikipedia.org/wiki/Knowledge_Discovery_in_Databases),
[SEMMA](https://en.wikipedia.org/wiki/SEMMA),
or [TDSP](https://medium.com/analytics-vidhya/tdsp-data-science-lifecycle-for-artificial-intelligence-data-science-projects-4ac6e5b82db4).
:::
...
- Celsius scale of temperature (there is no natural zero, 0°C is arbitrarily defined), a Celsius value $c$ can be linearly transformed to a Fahrenheit value $f$: $f = \frac{9}{5}c + 32$
...
- Kelvin (K) temperature, which has a true zero-point ($0\,K = -273.15\,^\circ C$): It is the point at which the particles that comprise matter have zero kinetic energy
...
| Nominal  | $=, \neq$                                         | X    | X         |                      |                      |
| Ordinal  | $=, \neq, <, >$                                   | X    | X         | X                    |                      |
| Interval | $=, \neq, <, >$, linear transformation             | X    | (X)       | X                    | X                    |
| Ratio    | $=, \neq, <, >, \times, /, +, -$                  | X    | (X)       | X                    | X                    |
...
```r
long_df <- long_df |>
  mutate(Quarter = str_remove(Quarter, "^Sales_"))
```
...
```r
integrated_data <- customers |>
  inner_join(sales, by = "Customer_ID")
```
...
- Modern machine learning and AI for big data were enabled by more computing power, new algorithms, new architectures, and new analytics processes.
