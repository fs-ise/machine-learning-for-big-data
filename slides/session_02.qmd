...
capabilities <- tribble(
  ...
  "Knowledge", 8, 9, "Reading & Writing", 6, 10, "Math", 4, 10,
  "Reasoning", 0, 7, "Working Memory", 2, 4, "Memory Storage", 0, 0,
  "Memory Retrieval", 4, 4, "Visual", 0, 4, "Auditory", 0, 6, "Speed", 3, 3
) |>
  pivot_longer(-capability, names_to = "model", values_to = "score") |>
  mutate(capability = factor(capability, levels = unique(capability)))
ggplot(capabilities, aes(capability, score, group = model, colour = model, fill = model)) +
  geom_polygon(alpha = .12) + geom_line(linewidth = 1) + geom_point() +
  coord_polar() + ylim(0, 10) + labs(title = "AI Capabilities", x = NULL, y = NULL) +
  theme_minimal()
```
:::
