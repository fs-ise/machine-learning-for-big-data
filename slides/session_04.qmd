...
title-slide-image: "../images/cover_slide/regression.png"
section_label: "Machine Learning Baselines: Regression"
session: "Session 4"
date: "2026-09-11"
format: revealjs
---
...
- **Describe** how regression models are implemented in R.
:::

<!--
, mapping conceptual components of the analytics process to their realization with `lm()` and the tidyverse
-->
...
As a first step, we retrieve the dataset and load it into our R environment for analysis.

```{r}
#| label: load-the-data-load-packages
#| echo: true
#| output: true
library(readr)
ames <- read_csv("../exercises/data/ames.csv", show_col_types = FALSE)
head(ames)
```
...
```{ojs}
//| label: regression-models-a-visual-illustration
n = 40
...
```{ojs}
//| label: regression-models-a-visual-illustration-inputs-table
Inputs.table(data, {
...
```{ojs}
//| label: regression-models-a-visual-illustration-viewof-combinedsliders
...
```{ojs}
//| label: regression-models-a-visual-illustration-plot-plot
Plot.plot({
  height: 480,
  marginLeft: 70,
  grid: true,
...
```{ojs}
//| label: interpreting-the-regression-model-exampledata
exampleData = [
...
```{ojs}
//| label: interpreting-the-regression-model-plot-plot
Plot.plot({
  height: 320,
  marginLeft: 70,
  grid: true,
...
Each coefficient ($\beta$) captures the **differential effect of a variable**—that is, how much the price is expected to change when that variable increases while the others are held constant.

<br>

```{dot}
//| label: multiple-predictor-model
...
predictors <- c("area", "Overall.Qual")
model <- lm(price ~ area + Overall.Qual, data = df)
model
```
...
<!--
    **TODO: clarify expectations (TBD: what is reasonable when ChatGPT would be available? - knowing code structure, reproducing? - add example exam questions)**
    ...
    ::: aside
    Data available at <https://www.openintro.org/data/index.php?data=ames>
    :::
-->
...
tidy(model) |>                          # coefficient estimates
  select(term, estimate)

glance(model) |>
  select(r.squared)                     # model fit (R²)
```
...
::: {.aside .fragment}
In R, `lm()` returns a fitted model object containing the estimated coefficients and
diagnostics. `broom::tidy()` and `broom::glance()` expose these results as tidy tables.
:::
...
::: notes
Main takeaway: students should understand $R^2$ well.
...
Because `lm()` provides inferential statistics, distinguish coefficient significance from predictive performance.
This helps students understand why statistical inference and predictive evaluation answer different questions.
:::
...
<!--
    ...
    see https://recipes.tidymodels.org/reference/step_dummy.html
...
# Load it later
loaded_model <- readRDS("house_price_model.rds")

new_house <- tibble(
  area = 1500,
  Overall.Qual = 7
)
predicted_price <- predict(loaded_model, newdata = new_house)
```
...
::: notes
You may mention that `saveRDS()` preserves a fitted R model as a single file.
...
- In practice, the analytics workflow can be implemented in R: define a model, fit it to data, generate predictions, and evaluate performance — a pattern that extends to many other modeling approaches.


{{< survey session_04 slide >}}
