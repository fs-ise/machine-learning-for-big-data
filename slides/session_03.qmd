...
library(tidyverse)
set.seed(42)
years <- c(rnorm(40000, 1965, 15), rnorm(20000, 1980, 8))
years <- years[years > 1920 & years < 2020]
ggplot(tibble(year = years), aes(year)) + geom_histogram(bins = 20) +
  labs(title = "Year of birth - Histogram", x = "Year of birth", y = "Frequency") + theme_minimal()
```
:::
...
```{r}
library(tidyverse)
tibble(field = factor(c("Computer Science", "Information Systems", "Business Administration", "Engineering"),
                      levels = rev(c("Computer Science", "Information Systems", "Business Administration", "Engineering"))),
       count = c(20, 25, 17, 12)) |>
  ggplot(aes(count, field)) + geom_col() + xlim(0, 30) +
  labs(title = "What is your background?", x = "Number of students", y = NULL) + theme_minimal()
```
:::
...
set.seed(42)
years <- c(rnorm(40000, 1965, 15), rnorm(20000, 1980, 8))
births <- tibble(year = years[years > 1920 & years < 2020])
p1 <- ggplot(births, aes(year)) + geom_histogram(bins = 20) +
  labs(title = "Year of birth - Histogram", x = NULL, y = "Frequency")
p2 <- ggplot(births, aes(year, "")) + geom_boxplot() + labs(x = "Year of birth", y = NULL)
p1 / p2
```
:::
...
set.seed(42)
savings <- tibble(
  group = rep(c("1A", "1B", "2A", "2B", "control"), each = 100),
  saving = c(rnorm(100, .6, .8), rnorm(100, .5, .9), rnorm(100, 1, 1.8),
             rnorm(100, .9, 1), rnorm(100, .4, .7))
)
ggplot(savings, aes(group, saving)) + geom_boxplot() + geom_hline(yintercept = 0) +
  labs(title = "Savings = cons. treatment - cons. baseline",
       x = "experiment group", y = "average el. consumption (kWh) per day") + theme_minimal()
```
:::
...
ggplot(d, aes(x, y)) + geom_point(colour = "#002EFF", alpha = .8) +
  labs(title = "Low correlation (r ≈ 0)") + theme_minimal()
```
...
library(ggplot2)
set.seed(1)
x <- rnorm(100); d <- data.frame(x, y = .8 * x + rnorm(100, sd = .3))
ggplot(d, aes(x, y)) + geom_point(colour = "#002EFF", alpha = .8) +
  labs(title = "High correlation (r ≈ 0.8)") + theme_minimal()
```
...
library(tidyverse)
set.seed(42)
centres <- matrix(c(-4, -4, -4, 4, 4, -4, 4, 4), ncol = 2, byrow = TRUE)
X <- map_dfr(1:4, ~ tibble(x = rnorm(75, centres[.x, 1], 1.2),
                           y = rnorm(75, centres[.x, 2], 1.2)))
plot_k <- function(k) {
  fit <- kmeans(X, centers = k, nstart = 25)
  ggplot(mutate(X, cluster = factor(fit$cluster)), aes(x, y, colour = cluster)) +
    geom_point(size = 1) +
    geom_point(data = as.data.frame(fit$centers), aes(x, y), inherit.aes = FALSE,
               shape = 4, colour = "red", size = 4, stroke = 1.5) +
    labs(title = paste0("k=", k), colour = NULL) + theme_void() + theme(legend.position = "none")
}
```
...
library(tidyverse)
ks <- 1:9
inertia <- map_dbl(ks, ~ kmeans(X, centers = .x, nstart = 25)$tot.withinss)
ggplot(tibble(k = ks, inertia), aes(k, inertia)) + geom_line() + geom_point() +
  labs(title = "Elbow method", y = "Within-cluster variance") + theme_minimal()
```
. . .
...
library(tidyverse)
ks <- 2:9
scores <- map_dbl(ks, function(k) {
  groups <- kmeans(X, centers = k, nstart = 25)$cluster
  mean(silhouette(groups, dist(X))[, "sil_width"])
})
ggplot(tibble(k = ks, score = scores), aes(k, score)) + geom_line() + geom_point() +
  labs(title = "Silhouette analysis", y = "Silhouette score") + theme_minimal()
```
...
plot(fit, main = "Dendrogram", xlab = "Observations", ylab = "Distance")
```
