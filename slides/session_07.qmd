...
ggplot(d,aes(team_size,experience,shape=class))+geom_point()+
...
d<-tibble(x=seq(1,15,length.out=60),y=-(x-8)^2+50+rnorm(60,0,3)); model<-ksvm(y~x,d,kernel="rbfdot",C=100,epsilon=2)
...
d<-tibble(x=seq(0,10,length.out=50),y=10*log1p(x)+rnorm(50,0,.5)); model<-ksvm(y~x,d,kernel="rbfdot",C=100,epsilon=.5)
...
d<-tibble(x=seq(1,20,length.out=60),y=100/(x+2)+rnorm(60,0,2)); model<-ksvm(y~x,d,kernel="rbfdot",C=100,epsilon=1)
