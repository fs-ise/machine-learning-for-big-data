...
title-slide-image: "../images/cover_slide/ml-algorithms.png"
section_label: "Machine Learning Foundations: Algorithms"
session: "Session 7"
date: "2026-09-25"
format: revealjs
---
...
- **Explain** how model-specific mechanisms constrain complexity and reduce overfitting, including regularization, tree depth, ensemble averaging, and margin control in SVMs.
...
Machine learning models can often handle both **classification** (predicting categories) and **regression** (predicting numeric values):
...
![](../images/high-dimensionality.png){height=450 fig-align=center}
...
Core idea of regularization: Prefer **simpler models** that fit the data *well enough*:

::: {.highlight_must_learn}

We modify the objective or loss function:
...
::: aside
In linear regression, the loss function optimized by OLS is $RSS=\sum_{i=1}^n(y_i - \hat{y}_i)^2$.
:::
...
| Regularization strength | Model complexity | Effect |
|:---:|-----|--------|
| $\lambda = 0$ | Relatively high | The penalty has no effect; ridge regression produces the ordinary least squares estimates. |
| $\lambda \rightarrow \infty$ | Lowest | The slope coefficients approach zero, so predictions approach the intercept: $\hat{y} = \beta_0$. |
...
source("../images/_comparison_decision_trees.R")
```
...
<div style="
  ...
  margin-top: 0.2em;
">
  <strong>Respond to Advertisement:</strong>
...
source("../images/_advertisement_decision_tree.R")
plot_advertisement_decision_tree()
```
...
Each internal node splits the data into **disjoint subsets** based on a feature and a **splitting criterion** (e.g., entropy, Gini index).
...
![](../images/decision_tree_illustration.png){height=420px fig-align=center}
...
| Name | Idea | Measure used | Type of splitting |
|:--|:-----|:---|:---|
| **CART** | Select the split that maximizes impurity reduction | Gini index | Binary |
| **ID3** | A foundational decision-tree algorithm that selects splits by maximizing information gain | Information gain (entropy) | Multiway; no pruning |
| **C5.0** | An extension of ID3 with improved splitting and pruning | Gain ratio | Binary or multiway; pruning |
| **CHAID** | Select the attribute most strongly associated with the target variable | Chi-square test | Multiway; pre-pruning |
| **Random forests** | Construct many trees using randomly selected samples and features; aggregate predictions by voting | Usually Gini index | Binary; typically unpruned |
...
/* Ensure the bottom border of the final data row is visible */
.tennis-table tbody tr:last-child td {
  border-bottom: 2px solid #333 !important;
}

.tennis-table tfoot td {
...
<table class="tennis-table">
  ...
  <tbody>
    <tr><td class="day">1</td><td>sunny</td><td>hot</td><td>high</td><td>light</td><td>no</td></tr>
    <tr><td class="day">2</td><td>sunny</td><td>hot</td><td>high</td><td>strong</td><td>no</td></tr>
    <tr><td class="day">3</td><td>overcast</td><td>hot</td><td>high</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">4</td><td>rainy</td><td>mild</td><td>high</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">5</td><td>rainy</td><td>cold</td><td>normal</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">6</td><td>rainy</td><td>cold</td><td>normal</td><td>strong</td><td>no</td></tr>
    <tr><td class="day">7</td><td>overcast</td><td>cold</td><td>normal</td><td>strong</td><td>yes</td></tr>
    <tr><td class="day">8</td><td>sunny</td><td>mild</td><td>high</td><td>light</td><td>no</td></tr>
    <tr><td class="day">9</td><td>sunny</td><td>cold</td><td>normal</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">10</td><td>rainy</td><td>mild</td><td>normal</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">11</td><td>sunny</td><td>mild</td><td>normal</td><td>strong</td><td>yes</td></tr>
    <tr><td class="day">12</td><td>overcast</td><td>mild</td><td>high</td><td>strong</td><td>yes</td></tr>
    <tr><td class="day">13</td><td>overcast</td><td>hot</td><td>normal</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">14</td><td>rainy</td><td>mild</td><td>high</td><td>strong</td><td>no</td></tr>
  </tbody>
  ...
  <tfoot>
    <tr>
      ...
      <!-- Features: columns 2–5 -->
      <td colspan="4" class="annotation">
        <svg class="brace"
             viewBox="0 0 100 14"
...
We take the attribute **Wind** as an example:

- **light**: 6 $\times$ yes, 2 $\times$ no
...
.wind-table tbody tr:last-child td {
  border-bottom: 2px solid #333 !important;
}
</style>
...
<table class="wind-table">
  ...
  <tbody>
    <tr><td class="day">1</td><td>light</td><td>no</td></tr>
    <tr><td class="day">2</td><td class="strong">strong</td><td>no</td></tr>
    <tr><td class="day">3</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">4</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">5</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">6</td><td class="strong">strong</td><td>no</td></tr>
    <tr><td class="day">7</td><td class="strong">strong</td><td>yes</td></tr>
    <tr><td class="day">8</td><td>light</td><td>no</td></tr>
    <tr><td class="day">9</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">10</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">11</td><td class="strong">strong</td><td>yes</td></tr>
    <tr><td class="day">12</td><td class="strong">strong</td><td>yes</td></tr>
    <tr><td class="day">13</td><td>light</td><td>yes</td></tr>
    <tr><td class="day">14</td><td class="strong">strong</td><td>no</td></tr>
  </tbody>
</table>
...
source("../images/_play_tennis_tree.R")
```
:::
...
- $t_i$ = number of observations with light or strong wind (8 resp. 6)
- t = total number of observations (14) and
...
At the root node, there are **9 × yes** and **5 × no**:

$$
\text{entropy(root)} =
- \left(
\frac{9}{14} \cdot \log_2\left(\frac{9}{14}\right)
+
...
This produces this decision tree:
...
source("../images/_play_tennis_id3_tree.R")
plot_play_tennis_id3_tree()
```
...
In the case of our example the information gain ratio of *wind* is

$$
\text{Information gain ratio (wind)} =
\frac{0.048}{
...
- $\text{temperature} < 71.5$: 4 $\times$ yes, 2 $\times$ no
...
- $\text{temperature} > 71.5$: 5 $\times$ yes, 3 $\times$ no
...
Regression trees predict a **numerical target variable** and can capture nonlinear relationships involving multiple predictors.

The figure shows a regression tree predicting the **natural logarithm of a baseball player's salary** (in thousands of dollars), based on:

- Years played in the major leagues
- Number of hits in the previous year

Each leaf predicts the **mean log-salary** of the training observations in that region.

. . . 

For players with Years < 4.5, the predicted log-salary is 5.11. We convert this back into dollars using the exponential function:

$$
\widehat{\text{Salary}} = 1{,}000 \cdot e^{5.11}
\approx \$165{,}670
$$

Players with Years $\geq 4.5$ are assigned to the right branch and further subdivided by Hits.

The predicted salaries for the resulting two groups are approximately:

- Hits < 117.5: $1{,}000 \cdot e^{6.00} \approx \$403{,}000$
- Hits $\geq$ 117.5: $1{,}000 \cdot e^{6.74} \approx \$846{,}000$
...
At each node, we evaluate **all available features $j$ and possible cutpoints $s$** and select the combination that minimizes the residual sum of squares (RSS):
...
where $\bar{y}_{R_1}$ and $\bar{y}_{R_2}$ are the mean target values of the training observations in the two resulting regions.

**Recursive splitting:** After selecting the best feature and cutpoint, we repeat this search independently within each child node, again considering all available features and cutpoints.

The process continues until a stopping criterion is reached (e.g., a minimum number of observations per leaf).

Each terminal region $R_1, \ldots, R_J$ predicts the mean target value of its training observations.

To reduce overfitting, tree complexity can be controlled through stopping criteria or pruning.
...
An important strength of random forests is that they are able to **perform well in the case of missing data**. According to their construction principle, not every tree is using the same features.
...
- $b$: bias (shifts the line/hyperplane)
...
## The key principle: maximize the margin
...
```{python}
#| label: the-key-insight-maximize-the-margin-import-numpy-as-np
#| fig-align: center
#| echo: false
...
from _svm_margin import plot_svm_margin
...
In the linear case, support vector regression (SVR) estimates the function

$$
\hat y_i = f(x_i) = \beta_0 + \sum_{j=1}^{p}\beta_j x_{ij}
$$

where $v_i = y_i - \hat y_i$ is the **residual (prediction error)** for observation $i$.

Nonlinear SVR can be obtained via kernels.

SVR combines two ideas to improve generalization:

1. **Error tolerance:** Prediction errors within an $\varepsilon$-tube are not penalized.
2. **Complexity control:** Penalize overly complex regression functions.
...
**OLS / Ridge: squared loss**

$$
L(v_i) = v_i^2
$$

**SVR: $\varepsilon$-insensitive loss**

$$
L_\varepsilon(v_i) =
\begin{cases}
0, & |v_i| \leq \varepsilon \\
|v_i| - \varepsilon, & |v_i| > \varepsilon
\end{cases}
$$

Errors inside the $\varepsilon$-tube are not penalized.

For observations outside the tube, the excess deviation is represented by the **slack variables** $\xi_i$.
:::

::: {.column width="50%"}
![](../images/svr_insensitive_loss_function.png){fig-align=center width=90%}
:::
...
The $\varepsilon$-insensitive loss treats errors according to their distance from the fitted function:

<br>

![](../images/svr_epsilon_insensitive_loss_function.png){fig-align=center width=70%}

- **Inside the tube:** Errors are not penalized.
- **Outside the tube:** The excess deviation beyond $\varepsilon$ is penalized and represented by $\xi_i$.
- Observations **on or outside the tube** can become support vectors and determine the fitted function.

## SVR objective

$$
\min_{\boldsymbol{\beta},\beta_0}
\frac{1}{2}\lVert\boldsymbol{\beta}\rVert^2
+
C\sum_{i=1}^{n}L_\varepsilon\!\left(y_i-f(x_i)\right)
$$

- $\frac{1}{2}\lVert\boldsymbol{\beta}\rVert^2$ controls model complexity.
- $\sum_i L_\varepsilon(y_i-f(x_i))$ penalizes errors outside the $\varepsilon$-tube.
- $\varepsilon$ controls the tube width.
- $C$ controls the trade-off between flatness and fitting violations.

## Kernel functions

Kernel Functions are used to project n-dimensional input to m-dimensional input, where m is higher than n:

![](../images/kernel_trick.png){fig-align=center width=50%}
...
A nonlinear pattern in the original, lower-dimensional space can become linearly separable in a higher-dimensional feature space.
...
```{python}
#| label: svr-examples-i-import-numpy-as-np
#| fig-align: center
import numpy as np
...
```{python}
#| label: svr-examples-ii-import-numpy-as-np
#| fig-align: center
import numpy as np
import matplotlib.pyplot as plt
from sklearn.svm import SVR
...
```{python}
#| label: svr-examples-iii-import-numpy-as-np
#| fig-align: center
import numpy as np
import matplotlib.pyplot as plt
from sklearn.svm import SVR
